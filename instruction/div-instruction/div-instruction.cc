...
void DivInstruction::execute(DataMemory& dataMemory, int& programCounter, OutputUnit& outputUnit, InputUnit& inputUnit) {
  ...
  if (operandType_ == "inmediate") {
    ...
    dataMemory.setRegister(kAccumulatorDirection, dataMemory.getRegister(kAccumulatorDirection) / dataMemory.getRegister(dataMemory.getRegister(operand_, position_)));
  }
  ++programCounter;
}